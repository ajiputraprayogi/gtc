...
$(function(){
    ...
    getTotal();
    keping();
    $('#datacifanggotabtn').click(function(){
        gethistorianggota();
    })
    getdataemasgtc();
    $('#btnmodaladdemasgtc').click(function(){
...
$("#btnaddpengajuan").on('click', function(e){
    var pengajuan = $('#pengajuan_hidden').val();
    var plafond_pinjaman = $('#plafond_pinjaman_hidden').val();
    if (parseInt(pengajuan) > parseInt(plafond_pinjaman)) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.emas, function (key, value) {
                ...
                rows = rows + '<td><input type="number" max="999" min="1" value="'+ value.keping +'" class="keping form-control" name="keping[]" placeholder="Qty" style="width: 90px;" data-toggle="input-mask" data-mask-format="00"></td>';
                rows = rows + '<td class="jumlah_gramasi">' + value.gramasi*value.keping + '</td>';
                rows = rows + '<td class="jumlah_buyback">' + (harga_buyback*value.keping).toLocaleString("id-ID") + '</td>';
                rows = rows + '<td hidden class="jumlah_buyback_hidden">' + harga_buyback*value.keping + '</td>';
...
function getTotal(){
    ...
    var numplafond_pinjaman = Number(newplafond_pinjaman);
    $("#plafond_pinjaman_hidden").val(numplafond_pinjaman).trigger('change');
    var pengajuan = $("#pengajuan").val();
    var newpengajuan = pengajuan.replace(/[^,\d]/g, '').toString();
    var numpengajuan = Number(newpengajuan);
    $("#pengajuan_hidden").val(numpengajuan).trigger('change');
    $('#pengajuan').bind("change keyup", function(){
        var numpengajuanhidden = Number($(this).val().replace(/[^,\d]/g, '').toString())
        $('#pengajuan_hidden').val(numpengajuanhidden).trigger("change")
